...
\begin{dokaz}
  ...
  \item Po definiciji interne logike regularne
    ...
    \item V interni logiki $\cat{C}$ lahko $y:Y \mid \exists (x:X) f(x) = y$
      izrazimo v diagramu
      % TODO fix v tem diagramu je en grd hack, da dvopičje deluje, ker se sicer ne compila
      \begin{equation*}
        \begin{tikzcd}
...
\begin{multline*}
    ...
    iz česar sledi
\begin{multline*}
  \{ \bar{x}:\bar{X}, \bar{w}:\bar{W} \mid \sigma\} \circ {\lbrace \bar{z}:\bar{Z},\bar{x}:\bar{X} \mid \eta\rbrace} \\
  =
    \{ \bar{x}:\bar{X}, \bar{w}:\bar{W} \mid \sigma \} \circ {\lbrace \bar{z}:\bar{Z},\bar{x}:\bar{X} \mid \theta \rbrace}
    \end{multline*}
    kar
    nam da enoličen morfizem $\{\bar{y}:\bar{Y} \mid \mu\}$, ki faktorizira
    $$\{ \bar{x}:\bar{X}, \bar{w}:\bar{W} \mid \sigma \},$$
    torej ${\lbrace \bar{x}:\bar{X},\bar{z}:\bar{Z} \mid \varphi \rbrace}$
    res določa kozožek $\{\Gamma ,\bar{x}:\bar{X} \mid \pi_1\}$ in
    $\{\Gamma ,\bar{x}:\bar{X} \mid \pi_2\}$. Definiramo
    ...
    ${\lbrace \bar{x}:\bar{X},\bar{z}:\bar{Z} \mid \varphi \rbrace}$ izenači
    ${\lbrace \bar{z}:\bar{Z},\bar{x}:\bar{X} \mid \eta\rbrace}$ in
    ${\lbrace \bar{z}:\bar{Z},\bar{x}:\bar{X} \mid \theta \rbrace}$, sledi
